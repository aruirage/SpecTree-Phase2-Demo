...
const changeStatuses = new Set(['新增', '删减', '变更']);
...
const CLAUSE_COMPARE_RECOVERY_KEY = 'daikin.clauseComparison.activeRun';
...
async function handleRun() {
  if (!oldFile.value || !newFile.value || !sharedSessionId.value) return;
  ...
  try {
    ...
    if (!res.ok) {
      let message = `比較に失敗しました (${res.status})`;
      try { const j = await res.json(); message = j.error || message; } catch {}
      throw new Error(message);
    }
    await res.json();
    const data = await waitForClauseCompareResult({ requestSeq, pollMs: 3000 });
    ...
    error.value  = e instanceof Error ? e.message : '比較に失敗しました。再試行してください';
    status.value = '';
  } finally {
...
async function waitForClauseCompareResult({ requestSeq, pollMs = 3000 } = {}) {
...
function handleStopRun() {
  ...
  if (sharedSessionId.value) {
    fetch('/api/clause-compare/cancel', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sessionId: sharedSessionId.value }),
    }).catch(err => console.error('Cancel clause comparison failed', err));
  }
}
...
function normalizeChangeStatus(clause) {
  ...
  if (changeStatuses.has(status)) {
    return detailedChangeTypesEnabled.value ? status : '变更';
  }
  if (status !== '有') return '';
  if (!detailedChangeTypesEnabled.value) return '变更';

  if (!clause.oldContent && clause.newContent) return '新增';
  if (clause.oldContent && !clause.newContent) return '删减';
  return '变更';
}

function rowClass(clause, idx) {
  const status = normalizeChangeStatus(clause);
  if (status === '新增') return 'row-added';
  if (status === '删减') return 'row-deleted';
  if (status === '变更') return 'row-changed';
  return idx % 2 === 0 ? 'row-even' : 'row-odd';
}

function statusClass(clause) {
  const status = normalizeChangeStatus(clause);
  if (status === '新增') return 'status-added';
  if (status === '删减') return 'status-deleted';
  if (status === '变更') return 'status-changed';
  return '';
}
...
function contentSegments(clause, side) {
  ...
  if (!text) return [];
  if (!detailedChangeTypesEnabled.value || normalizeChangeStatus(clause) !== '变更' || !clause.oldContent || !clause.newContent) {
...
const canRun = computed(() => !!oldFile.value && !!newFile.value && !running.value);

const filteredClauses = computed(() => {
  if (!result.value) return [];
  return showChangedOnly.value
    ? result.value.clauses.filter(c => isChanged(c))
...
function saveClauseCompareRecovery(isRunning = running.value) {
  if (!sharedSessionId.value) {
    sessionStorage.removeItem(CLAUSE_COMPARE_RECOVERY_KEY);
    return;
  }
  sessionStorage.setItem(CLAUSE_COMPARE_RECOVERY_KEY, JSON.stringify({
    sessionId: sharedSessionId.value,
...
function loadClauseCompareRecovery() {
  try {
    return JSON.parse(sessionStorage.getItem(CLAUSE_COMPARE_RECOVERY_KEY) || 'null');
  } catch {
    return null;
  }
}
...
async function restoreClauseCompareRecovery() {
  ...
  try {
    ...
    if (requestSeq !== runRequestSeq) return;
    stopRunProgress({ reset: true });
    error.value = e instanceof Error ? e.message : '比較結果の復元に失敗しました';
    status.value = '';
    saveClauseCompareRecovery(false);
  } finally {
    if (requestSeq === runRequestSeq) {
...
onMounted(restoreClauseCompareRecovery);
onActivated(restoreClauseCompareRecovery);
</script>
...
<template>
  <div class="clause-page">
    ...
    <aside class="left-panel">
      ...
      <div class="panel-body">
        ...
        <div class="section">
          ...
          <label
            class="upload-zone"
            :class="{ uploaded: !!oldFile }"
            @dragover.prevent
            @drop="onDropFile($event, 'old')"
          >
            <input type="file" accept=".pdf" style="display:none" @change="onFileChange($event, 'old')" />
            ...
            <template v-else>
              ...
              <div class="upload-zone-info">
                ...
                <span class="upload-zone-sub">PDF対応·単一ファイル上限：{{ MAX_UPLOAD_FILE_SIZE_MB }}MB</span>
              </div>
            </template>
          </label>
        </div>
        ...
        <div class="section">
          ...
          <label
            class="upload-zone"
            :class="{ uploaded: !!newFile }"
            @dragover.prevent
            @drop="onDropFile($event, 'new')"
          >
            <input type="file" accept=".pdf" style="display:none" @change="onFileChange($event, 'new')" />
        ...
        <!-- Status -->
        <div v-if="status && !error" class="status-msg">{{ running ? runProgressLabel : status }}</div>
        ...
        <div v-if="result" class="legend">
          ...
          <div v-if="detailedChangeTypesEnabled" class="legend-item">
            <span class="henko-mark status-added legend-mark">新增</span>
            <span class="legend-desc">新文件多出的条款</span>
          </div>
          <div v-if="detailedChangeTypesEnabled" class="legend-item">
            <span class="henko-mark status-deleted legend-mark">删减</span>
            <span class="legend-desc">新文件缺失的条款</span>
          </div>
          <div class="legend-item">
            <span class="henko-mark status-changed legend-mark">变更</span>
            <span class="legend-desc">利用規約が変更されました。</span>
          </div>
          <div class="legend-item">
    ...
    <!-- ── Right Panel ─────────────────────────────────────────────────── -->
    <div class="right-panel" :class="{ 'right-panel--fullscreen': resultFullscreen }">
      ...
      <div class="toolbar">
        ...
        <div class="toolbar-right">
          <button
            class="btn btn-sm"
            :class="showChangedOnly ? 'btn-filter-active' : 'btn-outline'"
            :disabled="!result"
          ...
          >
            ...
            {{ showChangedOnly ? '全件表示' : '変更のみ表示' }}
          </button>
          <button
            class="btn btn-outline btn-sm btn-icon"
            :disabled="!result"
            :title="resultFullscreen ? '退出全屏' : '全屏'"
            ...
            <template #dropdown>
              <el-dropdown-menu>
                ...
                <el-dropdown-item command="images">
                  <span class="download-menu-item">画像一括DL</span>
                </el-dropdown-item>
              </el-dropdown-menu>
            </template>
          </el-dropdown>
          ...
          <table class="comparison-table" :class="{ 'comparison-table--no-reference': !showImageColumns }">
            ...
            <thead>
              <tr class="thead-group">
                <th class="col-group-header col-group-old" :colspan="showImageColumns ? 3 : 2">【旧】REV</th>
                <th class="col-group-header col-group-new" :colspan="showImageColumns ? 3 : 2">【新】REV</th>
...
.comparison-table {
  ...
  .col-no      { width: 44px; }
  .col-clause  { width: 15px; }
  .col-content { width: 33%; }
  .col-attach  { width: 15px; }
  .col-henko   { width: 20px; }
}
...
/* Row states — row-level highlight only, no inline diff spans */
.row-added   { background: rgba(220,252,231,0.55); }
.row-deleted { background: rgba(254,226,226,0.55); }
.row-changed { background: rgba(255,237,213,0.55); }
